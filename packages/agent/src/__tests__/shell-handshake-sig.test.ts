...
describe('shell handshake signature binding (C1)', () => {
  ...
  it('a signature bound to ephemeral pair (A,B) does not verify against pair (A,C)', () => {
    ...
    const legitControllerEph = randomEphemeralPub(); // what controller sent
    const legitAgentEph = randomEphemeralPub(); // what agent sent (controller saw this)
    const attackerEph = randomEphemeralPub(); // what MITM sent to the other leg

    const timestamp = new Date().toISOString();

    // Controller signs over the transcript it saw: its own ephemeral + the
    // agent's ephemeral as forwarded by the (possibly malicious) relay.
    ...
    // Sanity: the signature verifies when the verifier reconstructs the same
    // transcript (both sides saw identical ephemerals).
    expect(verifyMessage(sig, msgForLegA, controller.publicKey)).toBe(true);

    // Attack: a MITM holds the controller's encrypted identity envelope from
    // leg A and re-encrypts it to the agent on leg B. The agent computes the
    // transcript from ITS ephemeral plus what the MITM actually sent on the
    // wire (attackerEph), not the controller's real ephemeral.
    const msgAsSeenByAgent = buildShellSigMessage({
      publicKey: controller.publicKeyBase64,
      deviceId: controller.deviceId,
      friendlyName: controller.friendlyName,
      timestamp,
      // Agent thinks the peer's ephemeral was attackerEph (what the MITM sent).
      signerEphPub: attackerEph,
      // Agent's own ephemeral — replace legitAgentEph with whatever the agent
      // picked; in the attack scenario it's unchanged from the agent's view.
      verifierEphPub: legitAgentEph,
    });
  ...
  it('domain separator prevents cross-protocol signature reuse', () => {
    // A signature over the shell-binding transcript should never validate
    // against a hand-crafted message that lacks the domain prefix.
    ...
    // The OLD format (pre-C1 fix) was just pub+name+timestamp concatenated.
    // A signature over the shell transcript must not be valid for the old
    // message form, or a future refactor could re-introduce the MITM.
