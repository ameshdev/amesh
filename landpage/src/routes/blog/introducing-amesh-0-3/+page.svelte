...
Identity created.
  ...
  Backend       : <span class="text-emerald-400">macOS Keychain</span>`} />
		...
		<ul class="space-y-3">
			...
			<li class="flex gap-3"><span class="text-emerald-400 mt-0.5">▸</span> <span><strong class="text-zinc-50">Device ID derivation consistency (0.3.3).</strong> <code class="font-mono text-sm">invite</code> and <code class="font-mono text-sm">listen</code> were deriving device IDs with raw <code class="font-mono text-sm">base64url(pubkey)</code> while <code class="font-mono text-sm">init</code> used <code class="font-mono text-sm">SHA-256(pubkey)</code> per the protocol spec. All commands now use <code class="font-mono text-sm">generateDeviceId()</code>. <strong>Existing pairings need re-pairing</strong> — this is the one migration note of the series.</span></li>
		</ul>
